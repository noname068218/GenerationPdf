...
namespace TemplateManager.APIs.DomainService
{
    ...
    /// </summary>
    internal class TemplateServiceApi : BaseServiceTemplate, ITemplateServiceApi
    {
        ...
        #region PROPERTIES
        private ITemplateApiBuilder ServiceBuilder { get => DomainServiceAPIs.GetBuilderFactory().GetTemplateApiBuilder(); }
        #endregion
        ...
        /// </summary>
        /// <param name="input">istanza dell'entità.<see cref="ITemplate"/>/param> 
        /// <param name="tokenAccess"> tokenAccess di validazione</param>
        /// <returns><see cref="FeedBackOperation"/></returns>
        public async Task<FeedBackOperation> SaveAsync(TemplateApi input, string tokenAccess = null)
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    ITemplate item = DomainServiceLocator.GetTemplateService().NewTemplate();
                    outup.Feedback = ServiceBuilder.Build(ref item, new TemplateApiBuilderArgs(input));
        ...
        /// <param name="tokenAccess"> tokenAccess di validazione</param>
        /// <returns></returns>
        public async Task<OutputTemplateApi> GetByIdAsync(int Id, TemplateApiSelectArgs args, string tokenAccess = null)
        {
            var outup = new OutputTemplateApi
            {
                Feedback = new BaseApiFeedback()
            };
            ...
            await Task.Run(() =>
            {
                try
                {
                    TemplateApi item = new();
                    item = GetById(Id, out BaseApiFeedback feed);

                    outup.Feedback = feed;

                    if (item != null)
                        outup.ListTemplate.Add(item);
                }
                catch (Exception ex)
                {
                    outup.Feedback.EVLogApi.MessageErrore = ex.Message;
        ...
        /// <param name="tokenAccess">tokenAccess di validazione</param>
        /// <returns></returns>
        public async Task<OutputTemplateApi> LoadListAsync(TemplateApiSearchCriteria searchCriteria, string tokenAccess = null)
        {
            var outup = new OutputTemplateApi
            {
                Feedback = new BaseApiFeedback()
            ...
            await Task.Run(() =>
            {
                try
                {
                    outup.ListTemplate = Search(searchCriteria, out BaseApiFeedback feed);
                    outup.Feedback = feed;
                }
                catch (Exception ex)
                {
