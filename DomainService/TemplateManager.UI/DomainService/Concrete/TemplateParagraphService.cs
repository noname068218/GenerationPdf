...
namespace TemplateManager.UI.DomainService
{
    ...
    /// </summary>
    internal class TemplateParagraphService : ITemplateParagraphService
    {
        ...
        /// <returns></returns>
        public FeedBackOperation SaveAsync(TemplateParagraphApi input, string tokenAccess)
        {
                ...
                try
                {
                    ...
                    string wsUrl = ExtensionMethodsUI.LoadUrlRestApi(BaseUrlRestApi.Anagraphic, ObjectModelName.TemplateParagraph, ActionOperationName.SaveAsync);
        ...
        /// <summary>
        /// Ritorna un'istanza dell'entità <see cref="TemplateParagraphApi" /> in base all'<paramref name="id" /> e
        /// agli argomenti specificati in <paramref name="args" />.
        /// </summary>
        /// <param name="id">id relativo al <see cref="TemplateParagraphApi" /></param>
        /// <returns>
        /// Istanza dell'entità <see cref="OutputTemplateParagraphApi" />; null se non esiste alcun TemplateParagraphQuantity con l'<paramref name="id"/> specificato.
        /// </returns>
        /// <exception cref="System.ArgumentException">se <paramref name="id"/> minore o uguale a 0.</exception>
        public OutputTemplateParagraphApi GetByIdAsync(int id, string tokenAccess)
        {
            OutputTemplateParagraphApi outputApi = new();
            try
            {
                var settings = new JsonSerializerSettings
                {
                ...
                string wsUrl = ExtensionMethodsUI.LoadUrlRestApi(BaseUrlRestApi.Anagraphic, ObjectModelName.TemplateParagraph, ActionOperationName.GetByIdAsync);
                string wsUrlInput = $"{wsUrl}?id={id}";

                var _response = ExtensionMethodsUI.GetAPIKeyAuthentication(wsUrlInput, tokenAccess);

                if (!string.IsNullOrWhiteSpace(_response))
                    outputApi = JsonConvert.DeserializeObject<OutputTemplateParagraphApi>(_response, settings);
        ...
        /// <summary>
        /// Elimina un'istanza dell'entità <see cref="TemplateParagraphApi"/> in base all'<paramref name="id"/>.
        /// </summary>
        /// <param name="id">id di un'istanza dell'entità <see cref="TemplateParagraphApi"/></param>
        ...
        public FeedBackOperation DeleteAsync(int id, string tokenAccess)
        {
            ...
            try
            {
                ...
                string wsUrl = ExtensionMethodsUI.LoadUrlRestApi(BaseUrlRestApi.Anagraphic, ObjectModelName.TemplateParagraph, ActionOperationName.DeleteAsync);
                string wsUrlInput = $"{wsUrl}?id={id}";
        ...
        /// <summary>
        /// Ritorna una lista di istanze dell'entità <see cref="TemplateParagraphApi"/>.
        /// </summary>
        /// <returns>
        /// lista di istanze dell'entità <see cref="TemplateParagraphApi" />
        /// </returns>
        /// <exception cref="System.ArgumentNullException">criteria</exception>
        public OutputTemplateParagraphApi LoadListAsync(TemplateParagraphApiSearchCriteria input, string tokenAccess)
        {
            OutputTemplateParagraphApi _output = new();
            try
            {
                var settings = new JsonSerializerSettings
                {
                ...
                string wsUrl = ExtensionMethodsUI.LoadUrlRestApi(BaseUrlRestApi.Anagraphic, ObjectModelName.TemplateParagraph, ActionOperationName.LoadListAsync);
                string inputJson = JsonConvert.SerializeObject(input, settings);

                var _response = ExtensionMethodsUI.PostAPIKeyAuthentication(wsUrl, inputJson, tokenAccess);

                if (!string.IsNullOrWhiteSpace(_response))
                    _output = JsonConvert.DeserializeObject<OutputTemplateParagraphApi>(_response, settings);
            }
            catch (Exception ex)
            {
